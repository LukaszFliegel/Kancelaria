...
<script type="text/javascript">
    ...
    function lgc<%=o %>() {  
        ...
        $(this).addClass('<%=sel %>').click(function(){
            $ae.<%= (Model.Data != null && Model.Data.ContainsKey("JSMethod") && Model.Data.Single(p => p.Key == "JSMethod").Value == "GetJSON" ? "JSONlookupChoose" : "lookupChoose") %>(
                '<%=o %>', 
                '<%=Url.Action((Model.Data != null && Model.Data.ContainsKey("JSMethod") ? Model.Data.Single(p => p.Key == "JSMethod").Value : "Get"), Model.Controller, new{Model.Area}) %>', 
                '<%=sel %>'
            );
        });
    }
    ...
    $(function () {
        ...
        $ae.popup('lp'+o, <%=Model.Width %>, <%=Model.Height %>, '<%=AwesomeTools.JsEncode(Model.Title) %>', true, 'center', true, {'<%=AwesomeTools.JsEncode(Model.ChooseText) %>': function () {
            $ae.<%= (Model.Data != null && Model.Data.ContainsKey("JSMethod") && Model.Data.Single(p => p.Key == "JSMethod").Value == "GetJSON" ? "JSONlookupChoose" : "lookupChoose") %>(
                '<%=o %>', 
                '<%=Url.Action((Model.Data != null && Model.Data.ContainsKey("JSMethod") ? Model.Data.Single(p => p.Key == "JSMethod").Value : "Get"), Model.Controller, new{Model.Area}) %>', 
                '<%=sel %>');
        },'<%=AwesomeTools.JsEncode(Model.CancelText) %>': function () { $(this).dialog('close'); }}, <%=Model.Fullscreen.ToString().ToLower() %>);
        //alert('po popupie');
        var lck<%=o%> = null;
...
<a class="ae-lookup-addbtn ui-state-default ui-corner-all" href="<%= Url.Action("Dodaj", Model.Controller ) %>" target="add<%: Model.Controller %>"><span class="ui-icon ui-icon-plusthick"></span></a>
<%if (Model.ClearButton)
  {%>
