var $ae = new function () {
    ...
    this.popup = function (o, w, h, title, modal, pos, res, btns, fulls) {
        ...
        $("#" + o).dialog({
            ...
            open: function (e, ui) {
                //alert('otwarlem okno: ' + $(this).attr('id'));
                var dlg = $(this);
                //alert('staram sie przestawic by za ' + dlg.parent().html() + ' byl ' + dlg.parent().next().html());
                //alert('staram sie przestawic');
                
                dlg.parent().before($(dlg.parent().next()));
    ...
    this.JSONloadLookupDisplay = function (o, url) {
        //alert('JSONloadLookupDisplay ' + url);
        $('#ld' + o).val('');
        var id = $('#' + o).val();
        if (id) $.getJSON(url, { id: id }, function (data) {
            //$("#ld" + o).val(d);
            //alert(data);
            $.each(data, function (key, val) {
                //items.push("<li id='" + key + "'>" + val + "</li>");
                //alert('czytam ' + key + ' o wartosci ' + val.Kod + ' oraz ' + val.Kwota);

                $("#ld" + o).val(val.Kod);
                // posatwienie kwoty tylko wtedy, gdy jest ona nieustawiona (rowna 0)
                //alert($("#Kwota").val());
                if($("#Kwota").val() == 0)
                    $("#Kwota").val(val.Kwota);
            });
    ...
    this.JSONlookupChoose = function (o, url, sel) {
        //alert('JSONlookupChoose');
        $('#' + o).val('');
        $('#' + o).val($('#' + o + 'ls .' + sel).attr("data-value")).change();
        this.JSONloadLookupDisplay(o, url);
        $("#lp" + o).dialog('close');
    }
